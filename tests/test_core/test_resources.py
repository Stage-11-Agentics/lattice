...
def _make_created_event(
    ...
    return create_resource_event(
        "resource_created", resource_id, actor, data,
        event_id="ev_CREATE000000000000000000", ts=ts,
    )
...
def _make_snapshot(
    ...
    event = _make_created_event(resource_id=resource_id, name=name,
                                 max_holders=max_holders, ttl_seconds=ttl_seconds)
...
class TestApplyResourceEvent:
    ...
    def test_resource_acquired(self) -> None:
        ...
        event = create_resource_event(
            "resource_acquired", _RES_ID, "agent:claude",
            {"holder": "agent:claude", "expires_at": _TS_LATER},
            event_id="ev_ACQ0000000000000000000000", ts=_TS_BASE,
        )
    ...
    def test_resource_acquired_with_task(self) -> None:
        ...
        event = create_resource_event(
            ...
            event_id="ev_ACQ0000000000000000000001", ts=_TS_BASE,
        )
        snap = apply_resource_event_to_snapshot(snap, event)
        assert snap["holders"][0]["task_id"] == "task_01TEST"

    def test_resource_released(self) -> None:
        snap = _make_snapshot(holders=[
            {"actor": "agent:claude", "acquired_at": _TS_BASE, "expires_at": _TS_LATER}
        ])
        event = create_resource_event(
            "resource_released", _RES_ID, "agent:claude",
            {"holder": "agent:claude"},
            event_id="ev_REL0000000000000000000000", ts=_TS_LATER,
        )
        snap = apply_resource_event_to_snapshot(snap, event)
        assert snap["holders"] == []
    ...
    def test_resource_heartbeat(self) -> None:
        ...
        event = create_resource_event(
            "resource_heartbeat", _RES_ID, "agent:claude",
            {"holder": "agent:claude", "expires_at": new_expires},
            event_id="ev_HB00000000000000000000000", ts=_TS_LATER,
        )
        snap = apply_resource_event_to_snapshot(snap, event)
    ...
    def test_resource_expired(self) -> None:
        ...
        event = create_resource_event(
            "resource_expired", _RES_ID, "agent:other",
            {"holder": "agent:claude", "expired_at": _TS_LATER, "reclaimed_by": "agent:other"},
            event_id="ev_EXP0000000000000000000000", ts=_TS_EXPIRED,
        )
        snap = apply_resource_event_to_snapshot(snap, event)
        assert snap["holders"] == []
    ...
    def test_resource_updated(self) -> None:
        snap = _make_snapshot()
        event = create_resource_event(
            "resource_updated", _RES_ID, "human:atin",
            {"field": "description", "old_value": None, "new_value": "Updated desc"},
            event_id="ev_UPD0000000000000000000000", ts=_TS_LATER,
        )
        snap = apply_resource_event_to_snapshot(snap, event)
